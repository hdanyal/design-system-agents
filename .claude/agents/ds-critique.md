...
Invoke via `/agents` or `@ds-critique`.
Stop and wait for an explicit yes before writes or spawn. Auto-approve is not kit confirm.
After yes, spawn only `.claude/agents/ds-critique.md` (or the confirmed next id) with the handoff path.
...
6. Propose `proposedLessons` in output when a standing bar should be recorded; write memory only after ack in a **later** turn.
...
working: …
proposedLessons: none | …
next: ds-architect | ds-coding | human
memory: ds-critique lesson | handoff-only
```
