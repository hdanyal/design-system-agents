...
1. Read generated \`.agents/skills/example-branding/reference.md\` before view, component, story, or token work.
...
4. Token edits route to \`example-update-design-language\`.
...
Forbidden: editing pack \`paths.ui\` for a new API or creating a new base without Architect rationale.`,
  "example-promote-block": `## Workflow
...
4. Stop for HITL. After HITL, **propose** a short catalog memory record for Documentation; do not write \`.agents/memory/shared/\` in the same turn as promotion. Template: \`references/catalog-fact.md\`.
Do not mark stable, promote unrelated view chrome, or commit \`public/r\`.`,
  "example-update-design-language": `## Workflow
1. Edit tokens.json or apply a preset as a candidate diff.
2. Run \`pnpm tokens:build\` and a11y/Foundations review.
3. Never restyle by editing stock or host component class colors.
4. Breaking token renames need a major Changeset.`,
  "example-update-shadcn": `## Workflow
...
Stop for CODEOWNER approval. Do not add the dependency first.`,
  "example-agent-tooling": `## Workflow
