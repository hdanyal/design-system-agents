...
8. **Stock UI** — do not restyle or fork pack `paths.ui`. If this host has `upstream-patches.json`, that ledger is the only allowed stock edit path; otherwise treat stock as read-only unless the pack says otherwise.
